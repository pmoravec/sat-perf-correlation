...
# Series, which makes impossible to add an indentation. The best way is
...
# as these are not important for finding triggers or symptoms
df.drop(
...
if args.load_type != "CPU":  # modify the column only when necessary
    ...
    for i in range(df_load["memory"].size-1, 0, -1):
        prev = current
        current = df_load["memory"][i]
        if prev == 0 or prev > current:
            df_load.at[i, "memory"] = 0
        else:
            df_load.at[i, "memory"] = current-prev
