...
# pmrep -p -t 60 -J10 -o csv -a $pmfile kernel.all.load mem.util.committed_AS proc.hog.cpu proc.hog.mem $stats > pmrep.input.csv
...
# some columns/statistics are irelevant for finding trigger/symptom behind a correlation, drop them:
DROP_COLUMN_SUFFIXES = ["-/median", "-/min", "-/percentile90", "-/percentile95", "-/percentile99", "-/std_deviation" ]
DROP_COLUMN_SUFFIXES_RE = rf"({'|'.join(DROP_COLUMN_SUFFIXES)})$"

# ignore these columns for computing and sorting correlation - it makes little sense to correlate 1m load to 5m load or similar
DROP_COLUMNS_FOR_CORREL = ['kernel.all.load', 'mem.util', 'statsd.pmda']
DROP_COLUMNS_FOR_CORREL_RE = rf"^({'|'.join(DROP_COLUMNS_FOR_CORREL)})"

# some columns are rather symptoms of a load than a trigger of the load itself - split them to optional comparison of symptoms correlation
COLUMNS_FOR_TRIGGERS_PREFIXES = ['statsd.fm_rails_http_request', 'statsd.fm_rails_importer_facts', 'openmetrics.foreman_tasks']
COLUMNS_FOR_TRIGGERS_PREFIXES_RE = rf"^({'|'.join(COLUMNS_FOR_TRIGGERS_PREFIXES)})"

# panda.Series strips leading blank characters of keys when printing whole Series which makes impossible to add indentation
# the best way is to implement own formatting of printing a Series
...
def find_correl_in_df(df_load, my_df, correl_type, load_type):
    ...
    if load_type != 'memory':
        ...
        metric_corr_cpu = my_df.corrwith(df_load["CPU"], numeric_only=True).fillna(value = 0).sort_values(ascending=False)[:args.items_limit]
        print_df_indended(metric_corr_cpu)
    if load_type != 'CPU':
        metric_corr_mem = my_df.corrwith(df_load["memory"], numeric_only=True).fillna(value = 0).sort_values(ascending=False)[:args.items_limit]
...
parser.add_argument("--load-type", choices=['CPU', 'memory', 'both'], default='both', help="What load types to find correlation? (CPU, mem, both)")
parser.add_argument("--input-csv", required=True, help="Input CSV file with PCP stats")
parser.add_argument("--items-limit", type=int, default=5, help="Limit of correlation items per a metric")
parser.add_argument("--detailed", action='store_true', default=False, help="Compare correlations to more detailed (less coarse) metrics (TODO)")
parser.add_argument("--show-symptoms", action='store_true', default=False, help="Show also correlations to symptoms, not only triggers")
parser.add_argument("--show-load-stats", action='store_true', default=False, help="Show statistics about the load (TODO)")
parser.add_argument("--peaks", action='store_true', default=False, help="Show correlation only to peaks of CPU/mem usage (TODO)")
...
if args.load_type != "CPU" and "mem.util.committed_AS" not in df.columns:
    print("Unable to see 'mem.util.committed_AS' metric in input data. Please re-run with --load-type=CPU")
    exit(1)
if args.load_type != "memory" and "kernel.all.load-1 minute" not in df.columns:
    print("Unable to see 'kernel.all.load-1 minute' metric in input data. Please re-run with --load-type=memory")
    exit(1)

# drop columns with /median, /min, /percentile or /std_deviation - these are not important for finding triggers or symptoms
df.drop(list(df.filter(regex=DROP_COLUMN_SUFFIXES_RE)), axis = 1, inplace = True)
# drop columns with zeroes only - they are irelevant for us and keeping them raises some CPython warning on execution
df = df.loc[:, (df != 0).any(axis=0)]
# split the DataFrame to potential triggers and symptoms, and also load (CPU/mem.consumtion)
df_triggers = df.filter(regex=COLUMNS_FOR_TRIGGERS_PREFIXES_RE)
df_symptoms = df.drop(columns=list(df.filter(regex=COLUMNS_FOR_TRIGGERS_PREFIXES_RE))).drop(columns=list(df.filter(regex=DROP_COLUMNS_FOR_CORREL_RE)))
df_load = df.filter(items=["kernel.all.load-1 minute", "mem.util.committed_AS"]).rename(columns={"kernel.all.load-1 minute": "CPU", "mem.util.committed_AS": "memory"})

# df_symptoms: aggregate "proc.hog.cpu *" by process name, and "proc.hog.mem *" the same
...
for pivot in ["proc.hog.cpu", "proc.hog.mem"]:
    ...
    for group in list(set(cols.map(lambda x: x.split()[-1]))):
        proc_df[f"{pivot}:{group}"] = pivot_df.filter(regex=fr'{group}').sum(axis=1)
df_symptoms = df_symptoms.join(proc_df)

# modify memory consumption to memory increase - see explanation before split_long_duration_to_past method
# we must update the values from newest to oldest as for updating i-th value, we need the original (i-1)th
...
if args.load_type != "CPU": # modify the column only when necessary
    for i in range(df_load["memory"].size-1, -1, -1):
        df_load.at[i, "memory"] = \
            0 if (i == 0 or df_load["memory"][i-1] == 0 or \
                  df_load["memory"][i-1] > df_load["memory"][i]) \
            else df_load["memory"][i]-df_load["memory"][i-1]
